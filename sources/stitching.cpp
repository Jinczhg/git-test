...
void Stitching::iteration(string outDir, int idx)
{
	//dilate_times = 1.5;
	...
	while (!(H_flag&&Pass_flag)){//当H不符合要求，或者缝隙没有穿过配准区域时，循环		
		...
		if (seam_section.size() == 0)
			Dilate_flag = true;
		if (Dilate_flag){//触发Dilate_flag，扩大缝隙段邻域范围，重新迭代
			//dilate_times += 0.5;
			_sigma_spatial += 5;
			cout << endl;
			cout << "sigma_spatial :" << _sigma_spatial << endl;
			mask0_avoid.setTo(Scalar(0));
			mask1_avoid.setTo(Scalar(0));
...
void Stitching::Stitch(string outDir)
{
